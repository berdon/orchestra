...
- Fix the shared search helper in `src/lib/referenceMentions.ts`, keep canonical insertion as `@TASK-NUMBER`, and add regression coverage for comments + chat.
...
3. **Ranking should prefer precise task lookup before fuzzy discovery:**
   - exact number / exact slug
   - prefix number / prefix slug
   - title word / title substring
   - fuzzy title fallback
   - then agent / role matches
4. **Result capping should not hide precise task matches.**
...
- `tests/referenceMentions.test.ts`
  - slug exact match
  - slug prefix match
  - number exact/prefix match
  - >12 matching tasks still surface all precise task matches
  - stable insertion remains `@TASK-NUMBER`
