...
test("agents page can open an idle agent session in terminal mode and restores chat when the terminal closes", async ({ page }) => {
  await page.addInitScript(() => {
    window.localStorage.clear();
  });
  ...
  await page.getByRole("button", { name: "Agents" }).click();
  await page.getByRole("link", { name: /Data/i }).click();
  await page.locator('[data-role="open-agent-session-terminal"]').click();
...
test("agents page keeps the terminal option disabled while an agent is already busy", async ({ page }) => {
  await page.addInitScript(() => {
    window.localStorage.clear();
    ...
    window.localStorage.setItem(
      ...
      JSON.stringify([
        ...
        },
        {
          ...
          slug: "data",
          name: "Data",
          description: "Busy agent runtime.",
          systemPrompt: null,
          provider: null,
          model: null,
          roleId: null,
    ...
    window.localStorage.setItem(
      ...
      JSON.stringify([
        {
          projectId: "orchestra",
          agentId: "agent-data-busy",
          status: "running",
          mainSessionId: "session-busy",
          runtimeCwd: "/mock/projects/orchestra",
  ...
  await page.getByRole("button", { name: "Agents" }).click();
  await page.getByRole("link", { name: /Data/i }).click();
  await expect(page.locator('[data-role="open-agent-session-terminal"]')).toBeDisabled();
});
