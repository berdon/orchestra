...
async function getTranscriptSnapshot(webdriverSessionId: string) {
  return executeScript<TranscriptSnapshot>(
    webdriverSessionId,
    `
      const rows = Array.from(document.querySelectorAll('[data-role="transcript-event"]')).map((node) => {
      ...
      return {
        ...
        transcriptText: document.querySelector('[data-role="session-transcript"]')?.textContent?.replace(/\s+/g, ' ').trim() || '',
      };
    `,
  );
}
...
function userMessageCount(record: SessionRecordLike | null | undefined) {
  return (record?.events ?? []).filter((event) => event.kind === "user" && Boolean((event.message ?? "").trim())).length;
}

function assistantMessageCount(record: SessionRecordLike | null | undefined) {
  return (record?.events ?? []).filter((event) => event.kind === "assistant" && Boolean((event.message ?? "").trim())).length;
}
...
function firstIndexContaining(rows: Array<{ kind?: string; text?: string | null; message?: string | null }>, kind: string, needle: string, startIndex = 0) {
  const normalizedNeedle = normalizeComparableText(needle);
  return rows.findIndex((row, index) => index >= startIndex && row.kind === kind && normalizeComparableText(row.text ?? row.message).includes(normalizedNeedle));
}

function expectUserAssistantOrdering(record: SessionRecordLike, prompts: string[]) {
  ...
  for (let index = 0; index < prompts.length; index += 1) {
    ...
    expect(userIndex, `expected persisted user message for ${prompt}`).toBeGreaterThanOrEqual(0);
    const nextUserIndex = index + 1 < prompts.length
    ...
    const assistantIndex = rows.findIndex((row, rowIndex) => rowIndex > userIndex && rowIndex < assistantWindowEnd && row.kind === "assistant" && Boolean(row.message.trim()));
    expect(assistantIndex, `expected an assistant reply after ${prompt}`).toBeGreaterThan(userIndex);
    cursor = userIndex + 1;
  }
}

function expectTranscriptLifecycleOrder(snapshot: TranscriptSnapshot, expectedKinds: string[]) {
...
async function createFreshSession(webdriverSessionId: string) {
  ...
  const snapshot = await waitForConditionWithDiagnostics(
    ...
    (value) => Boolean(value.selectedSessionId) && !sessionsBeforeCreate.some((session) => session.id === value.selectedSessionId),
...
async function sendComposerMessage(webdriverSessionId: string, message: string) {
  await setInputValue(webdriverSessionId, '[data-role="composer-input"]', message);
  await clickSelector(webdriverSessionId, '[data-role="send-message"]');
  await waitForText(webdriverSessionId, message, 15_000);
}

async function waitForPendingAssistant(webdriverSessionId: string, sessionId: string) {
  return waitForConditionWithDiagnostics(
    webdriverSessionId,
    sessionId,
    () => getTranscriptSnapshot(webdriverSessionId),
    (snapshot) => snapshot.rows.some((row) => row.kind === "assistant" && row.pending),
...
async function waitForSessionSettled(
  ...
  return waitForConditionWithDiagnostics(
    ...
    async () => ({
      ...
      record: await invokeCommand<SessionRecordLike>(webdriverSessionId, "get_session_record", { sessionId }),
    }),
    ({ snapshot, record }) => record.status === "idle"
...
describe("desktop session message lifecycle", () => {
  it.skipIf(!isDesktopE2E)("resolves a pending assistant placeholder and preserves the same settled transcript after a page reload", async () => {
    ...
    try {
      ...
      const settledSequence = settled.snapshot.rows.map((row) => ({ kind: row.kind, pending: row.pending, text: row.text }));
      ...
      await clickSelector(webdriverSessionId, `[data-role="session-link"][data-session-id="${sessionId}"]`);
      const reloaded = await waitForConditionWithDiagnostics(
        ...
        ({ snapshot, record }) => snapshot.selectedSessionId === sessionId && record.status === "idle" && snapshot.pendingCount === 0,
        "reloaded transcript to settle without stale pending state",
        60_000,
      );

      expect(reloaded.snapshot.rows.map((row) => ({ kind: row.kind, pending: row.pending, text: row.text }))).toEqual(settledSequence);
  ...
  it.skipIf(!isDesktopE2E)("keeps multiple sequential sends ordered and settled in both the transcript and backend record", async () => {
    const webdriverSessionId = await createReadyWebdriverSession();
    ...
    try {
      ...
      const finalState = await waitForConditionWithDiagnostics(
        ...
        ({ snapshot, record }) => snapshot.pendingCount === 0 && record.status === "idle",
        "sequential send transcript to remain settled",
      );

      expectTranscriptLifecycleOrder(finalState.snapshot, ["user", "assistant", "user", "assistant"]);
  ...
  it.skipIf(!isDesktopE2E)("keeps a follow-up send visible while the prior response is still pending and settles both turns without stale pending state", async () => {
    const webdriverSessionId = await createReadyWebdriverSession();
    ...
    try {
      ...
      const pendingSnapshot = await waitForConditionWithDiagnostics(
        ...
        sessionId,
        () => getTranscriptSnapshot(webdriverSessionId),
        (snapshot) => snapshot.rows.filter((row) => row.kind === "user").length >= 2,
        "queued follow-up message to stay visible while a prior response is pending",
        30_000,
        250,
      );
      expect(pendingSnapshot.rows.filter((row) => row.kind === "user").length).toBeGreaterThanOrEqual(2);
      ...
      expectTranscriptLifecycleOrder(settled.snapshot, ["user", "assistant", "user", "assistant"]);
  ...
  it.skipIf(!isDesktopE2E)("clears pending message state after stopping an in-flight response", async () => {
    ...
    try {
      ...
      await waitForConditionWithDiagnostics(
        ...
        (record) => (record.events ?? []).some((event) => event.kind === "user" && (event.message ?? "").includes("Use the bash tool exactly once")),
      ...
      const stopped = await waitForConditionWithDiagnostics(
        ...
        ({ snapshot, record }) => record.status === "paused"
          && snapshot.pendingCount === 0
          && (record.events ?? []).some((event) => (event.message ?? "").includes("Session run stopped by operator.")),
      ...
      expect(stopped.snapshot.rows.some((row) => row.eventId.startsWith("client-stop-"))).toBe(false);
      expect(
        stopped.record.events?.some((event) => (event.message ?? "").includes("Session run stopped by operator.")),
      ).toBe(true);
    } finally {
      await deleteWebdriverSession(webdriverSessionId);
    }
