...
const logPath =
  process.env.ORCHESTRA_FAKE_PI_RESUME_CONTINUATION_LOG_PATH || "";
const sessionId = path.basename(sessionFile, path.extname(sessionFile));
const defaultModel = {
  id: "fixture-model",
  name: "Resume Continuation Fixture",
  provider: "fixture",
  api: "fixture-rpc",
  reasoning: false,
};
...
function ensureSessionHeader() {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      type: "session_info",
      id: sessionId,
      name: "Fake Pi resume continuation fixture session",
...
function writeAssistantReply(commandType, message) {
  ...
  const assistantText = `Fixture ${commandType} reply: ${String(message || "")}`;
  ...
  writeResponse({
    ...
    assistantMessageEvent: {
      type: "text_delta",
      contentIndex: 0,
      delta: `Fixture ${commandType} reply: `,
      partial: {},
    },
  });
  writeResponse({
...
function handleCommand(command) {
  ...
  if (command.type === "prompt") {
    ...
    writeAssistantReply("prompt", command.message);
    return;
  }

  if (command.type === "follow_up" || command.type === "steer") {
    appendLog({
      branch: `${command.type}:idle-noop`,
      message: typeof command.message === "string" ? command.message : "",
    });
    writeResponse({
      id: command.id,
